...
class TransformerLoss_P(nn.Module):
    def __init__(self):
        super(TransformerLoss_P, self).__init__()
        
    def forward(self, outputs, targets):
        mel_out, mel_out_post, enc_alignments, dec_alignments, enc_dec_alignments, gate_out = outputs
        mel_target, gate_target, text_lengths, mel_lengths = targets
        mel_target, gate_target = mel_target[:,:,:mel_out.size(2)], gate_target[:,:mel_out.size(2)]
        ...
        mel_loss = nn.L1Loss()(mel_out, mel_target) + nn.L1Loss()(mel_out_post, mel_target)
        bce_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(5.0))(gate_out, gate_target)
        guide_loss = self.guide_loss(enc_dec_alignments, text_lengths, mel_lengths)
